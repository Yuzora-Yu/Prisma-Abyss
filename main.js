    ...
    constructor(data) {
        ...
        if(data.charId) {
            ...
            if(master && master.lbSkills) {
                Object.entries(master.lbSkills).forEach(([requiredLb, skillId]) => {
                    const required = Math.max(0, Math.floor(Number(requiredLb) || 0));
                    if (skillId && this.limitBreak >= required) this.learnSkill(skillId);
                });
            }
        }
...
スカイプリズムを1個消費します。`, () => {
    ...
    // 完了済みのゼノンクエストに紐づく旧戦後ボス描画コンテキストを一度だけ掃除する。
...
一日に一度だけ、水の加護から小さな返礼を受けられます。`);
    ...
    getBattleLimitBreakSteps: (battleWins) => {
        ...
        const interval = Math.max(1, Math.floor(Number(App.limitBreakConfig.battlesPerStep) || 20));
        const max = Math.max(0, Math.floor(Number(App.limitBreakConfig.heroBattleMax) || 70));
    ...
    ensureLimitBreakProgress: (char) => {
        ...
        const retiredDungeonSource = Math.max(0, Math.floor(Number(p.sources.dungeon) || 0));
        const sourceKeys = ['story', 'battle', 'quest', 'boss', 'prism', 'random', 'gacha', 'monster', 'trial', 'item', 'legacy'];
        sourceKeys.forEach(key => {
            p.sources[key] = Math.max(0, Math.floor(Number(p.sources[key]) || 0));
        });
        // 探索深度によるLB獲得は廃止。旧セーブに残る値だけlegacyへ一度移してキー自体を除去する。
        if (retiredDungeonSource > 0) p.sources.legacy += retiredDungeonSource;
        delete p.sources.dungeon;
    ...
    isFieldControlBlocked: () => {
        ...
        if (typeof StoryManager !== 'undefined') {
            ...
            if (typeof StoryManager.hasPendingFieldResume === 'function' && StoryManager.hasPendingFieldResume()) return true;
        }
...
load: () => { 
    ...
    applyLevelUpGrowth: (charData, options = {}) => {
        ...
        if (charData.level === 50 || charData.level === 100) {
            // 表示Lv基準なので転生後の各周回でも再発生する節目成長。計算式は転生回数に関係なく維持する。
	 ...
	 *
	 * ※ 転生時は「表示Lv1に戻る」が、
	 *    必要EXPの基礎計算は今後も effectiveLevel = level + 転生回数*100 を不変の前提とする。
	 * ※ ただし各周回の表示Lv49→50 / Lv99→100では、同じ壁倍率を再適用する。
	 */
	getNextExp: (charData) => {
		...
		// --- 101+（転生帯）：実効Lv曲線。式は維持し、係数のみ全体バランス調整の対象とする ---
		const P_REINC = 0.6;
		...
		if (eL <= 10) {
		  ...
		  // 基礎曲線は実効Lvで計算する。壁倍率は後段で表示Lvを見て適用する。
		  ...
		  // 転生後も旧式どおり実効Lv101+の曲線をそのまま継続する。
		  const step101 = base100 * REINC_STEP_RATE;
		  needExp = base100 + step101 * Math.pow(eL - 100, P_REINC);
		}

		// 周回ごとの節目。実効Lvの基礎曲線は変えず、表示Lv49/99にだけ壁を再適用する。
		// これにより転生2回目以降も 49→50 / 99→100 が育成上の明確な節目になる。
		if (level === 49) needExp *= WALL_50;
		else if (level === 99) needExp *= WALL_100;
...
const Field = {
    ...
    updateHeldKeyHud: () => {
        ...
        const signature = `${canShow ? 'visible' : 'hidden'}:${colors.join(',')}`;
        // drawHudMinimap() runs after every field render. Rebuilding identical IMG
        // nodes each time creates avoidable DOM work without changing the HUD.
        if (Field._heldKeyHudSignature === signature) return;
        Field._heldKeyHudSignature = signature;
        const keyDefs = {
