...
3. 転生の実（Item 107）は通常ランダム宝箱候補から除外。後続補正により、超レア枠では「転生の実／神鉄の鍛冶台／合成の壺／災厄の楔」の4種から出現する。
...
Item 107 の `randomChestDrop:false` は維持し、通常のランダムアイテム候補からは除外する。

2026-08-15後続補正で、ランダム宝箱の**超レア枠**だけは次の4種専用プールへ統一した。
...
超レア枠は赤黒フラッシュを伴う。固定配置宝箱、実績報酬、施設交換、明示的な報酬・ドロップは変更していない。
