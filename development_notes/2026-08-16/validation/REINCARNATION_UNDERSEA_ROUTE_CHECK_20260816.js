...
ok('undersea volcano coordinate is sea tile', world[17][113] === 'W');
...
ok('undersea volcano is point-symmetric opposite lighthouse around palace',
  volcanoArea.centerX === palace.centerX * 2 - lighthouse.centerX && volcanoArea.centerY === palace.centerY * 2 - lighthouse.centerY);
ok('undersea volcano world marker waits for route flag', volcanoArea.worldConditions?.requiredFlag === 'underseaVolcanoRouteOpened');
...
ok('undersea volcano hidden before route-open flag', registryCtx.MapRegistry.getWorldAreaAt(113,17) === null);
registryCtx.App.data.progress.flags.underseaVolcanoRouteOpened = true;
ok('undersea volcano resolves on world map after route-open flag', registryCtx.MapRegistry.getWorldAreaAt(113,17)?.[0] === 'UNDERSEA_VOLCANO');
...
ok('undersea volcano first floor exits to sea', volcano.floors?.[0]?.proceduralExitLabel === '海上へ戻る');
...
ok('objective tells exact undersea volcano direction', sd.storyObjectives['6-2'].includes('光の宮殿の北東') && sd.storyObjectives['6-2'].includes('大灯台と反対側'));
...
ok('service-worker app-shell cache version bumped', swSource.includes('prisma-abyss-v66.20260816'));
