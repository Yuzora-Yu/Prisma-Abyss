...
#### ボス戦のAUTO運用（2026-08-17追記）

- 現存するボス戦は、通常ボス・イベントボス・強制敗北／耐久型ボスを含めてすべてAUTO使用可能とする。
- ボス戦開始時には既存の共通処理でAUTOを一度OFFへ戻す。これは重要戦闘の開始をプレイヤーへ認識させるための開始時挙動であり、戦闘中のAUTO再ONは禁止しない。
- `forceAutoOff` によるAUTOボタンのロック機構自体は、将来どうしても手動操作を要求する特殊戦を作る可能性に備えて削除しない。ただし現存ボスのイベント定義には設定しない。
- AUTO禁止が必要な新規戦闘を将来追加する場合は、通常のボス難度を理由に使用せず、その戦闘固有の操作ギミック上不可欠であることを正本へ明記してから使用する。
...
## Logic Review Priority (2026-08-17)

状態遷移を伴う実装の検証は、**コード経路を人間が入口から出口まで目視で追うことを主、ツール検証を補助**とする。

- 正常系だけでなく、失敗、再試行、全滅、再読込、中断、MAP遷移後の再開まで同じ状態機械として追う。
- `EVENT` の入れ子、MAP遷移、Scene Context、一時パーティ、強制敗北、checkpoint、save抑止は個別機能として切り離さず、開始から復帰まで一続きの処理として確認する。
- syntax/schema/file-existence validatorは必要最小限の補助確認に使う。validatorのPASSだけを「ロジックが生きている」根拠にしない。
- 遷移命令では「APIを呼べたか」だけでなく、目的地へ到達後にイベントの残り命令が再開されるか、失敗時に元の命令を再試行できるかまで確認する。
- 回想・特殊戦闘では、通常全滅とシナリオ上の強制敗北を別経路として追い、復帰地点・イベントキュー・現在世界への復元が競合しないことを確認する。
