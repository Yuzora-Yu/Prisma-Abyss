...
## 2026-08-12 水上都市～船取得ルート確定

海底神殿後の中盤導線は以下を正とする。

1. 海底神殿クリア。
2. ソフィアからケイトへ水を介した念話。水上都市で暗黒騎士暴動。
3. 水上都市内で5戦し、最後にクロード／レオン率いる聖騎士団が到着して鎮圧。
4. 暴動後、水上都市とカザリアで本名を伏せた「聖女」の噂を薄く出す。
5. カザリアからアリサ／ハイネ救援要請。ソフィアは船の持ち主調査を並行する。
6. 禁忌の森深部で二人を救出し、本編必須加入。
7. 水上都市へ戻り、ソフィアから旧レクスノート侯爵家の船を聞く。ハイネはアレル侯から仕事を請けたことがある。
8. レクスノート邸でアランから地下迷宮調査を依頼される。B1～B4は再入場で構造が変化する可変階層（通常敵Rank40～49、レア枠メタルジェリー）。B5は固定「レクスノート邸 隠し書庫」。
9. B5で魔導司書レグルス（301033 / Rank45）を撃破し、貴重品「レクスノートの魔道書」（Item 701013）を取得。アランへ報告後、船取得・アラン正式加入。

アリサはヴェリア出身で、風へ旋律を捧げてきた楽師の家系の血を引く。カザリア滞在中に森奥の神殿から風の音を感じ、風のプリズムへ礼拝に向かった後、さらに風に導かれて禁忌の森深部へ進んでしまう。深部ボス後には貴重品 **「古びた魔笛」（Item 701012）** を取得する。これはヴェリア祭祀文化へつながる物証だが、この時点では出自を断定せず、アリサの指が笛穴を覚えている描写だけを置く。

クロードは王国側の責任をまず引き受け、事実としてアルスの救助行動を認める。レオンは民を守ることを騎士の本義としながら、国策を無視して自由に動くアルスへ反発と嫉妬を抱く。
...
## 2026-08-10 Phase8F latest override — 災禍の根ジャゴレア

- 深淵90階相当ジャスパー戦は、混沌呪縛罠の後にジャスパーが自身の計画を自慢げに自白する。
- 統合の祭壇でアラン死亡: 不意打ち＋主要能力半減でジャスパー戦。
- アラン生存: アランが混沌呪縛を崩し、主人公能力参照の戦闘外NPC援護として毎ターン参加する。
- アラン援護技: アステリア / 霊脈断ち / 戦神の律動 / ルクシオン・ノナ。
- ジャスパー撃破後、アラン再加入をプレイヤーが改めて許可／保留する。
- 許可時は即加入。保留時は混沌魔城レガシオンで後日加入可能。
- 実加入時Story EXPは+1,000,000 once-only。旧+500,000案は上書き。
- 詳細正本: `43_JAGOREA_JASPER_ALAN_SUPPORT_PHASE8F_20260810.md`。
