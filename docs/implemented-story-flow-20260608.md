...
- 大灯台攻略後、回想イベントを経て現在時間の光の宮殿攻略へ進む。
- 光の宮殿回想3Fの六芒星・ジャスパー・ヴェルドはMAP絶対座標を正本とし、Phaser停止時もCanvas fallbackで位置と表示を維持する。六芒星画像本体もruntimeへ同梱し、X17/Y16・9タイル規模で表示する。封印後にヴェルドへ踏み込む際のみレイラを中央列へ寄せる。封印床と退却制止床が重なる箇所は、現在の進行条件に合うstoryEventを選択してクロードの逆走制止を維持する。
- 固定ダンジョンは通常1〜4階に加え、地下牢・聖女の部屋・宝物保管庫を持つ。
- 光の宮殿グランプリズマ周辺のワールド遭遇は `LIGHT_PALACE_GROVE`（Rank61 / `MAP000022`）を正本とする。遭遇プロファイル／戦闘データ間で未指定のRank上下限 `null` を `0` へ数値化しないよう修正し、Rank1範囲への誤変換を防ぐ。
- 回想中はScene Contextで専用編成／所持品を隔離するが、編成操作自体はロックしない。Scene Contextの復元snapshotもセーブpayloadへ保持し、回想中の再読込・手動ロードでも現在位置から継続できる。Phase 10以前のsnapshotなし回想セーブも、同進行段階の通常セーブが残っていれば復元元として自動再構成する。回想専用Lvはレイラ49、レオン62、クロード58（ルーナは既存設定維持）。
- 回想3階の六芒星イベント（X16〜18 / Y19）でルーナ `401` が封印され、回想中の一時パーティと仲間一覧から離脱する。X16/X18で着火してもレイラをX17/Y19へ寄せてから進行し、六芒星はX17/Y16へ9タイル規模、ジャスパーはX20/Y15、ヴェルドはX17/Y16のMAP絶対座標で固定描画する。床演出はPhaser起動・textureロード待ちでもrenderer側にpending仕様を保持し、再読込・リサイズ後も同じMAP座標へ再同期する。レイラ対ヴェルド戦後はフラッシュボムから約3マス南へ退避してから操作を戻し、X16〜18 / Y19へ北上し直すとクロードが制止する。さらに一度下りた階へは上り直せず、2階→3階・1階→2階の逆走もクロードが止める。回想終了時はScene Context復元を行い、旧回想セーブの復元元が不正でも雷の要塞1Fへ帰還する互換制約を適用する。
- 回想中の一時キャラクター状態は実績進行へ使用しない。ヴェルド初戦／入口戦は5ターンで自動終了せず、5ターン経過またはHP1到達の早い方で「黒白の葬閃」演出へ入り、必中固定9999ダメージで敗北演出へ進む。 回想最後の記憶文の後は上下から暗転し、完全暗転中にScene Contextを終了して雷の要塞へ戻してから現在時間を表示する。
- 現在時間では、地下牢の主要生存者確認前に1階から2階へ上がろうとするとジョセフがレオン／レイラの安否確認を優先するよう促し、地下牢へ向かうまで上階進行を止める。地下牢の主要生存者確認後に上階進行と祭壇戦が解禁される。
- 4階祭壇にはジャスパー `301070` とヴェルド `301050` を個別配置し、どちらを調べても `light_palace_final_encounter` を開始する。
- 初戦は `bossStatMultiplier: 3`。勝利時は `light_palace_alan_betrayal` へ直行する。
- 初戦敗北時はGame Overにせず `light_palace_blessing_retry` へ進み、既存会話と全回復後に通常倍率で再戦する。再戦勝利後も `light_palace_alan_betrayal` へ合流する。
- 旧 `light_palace_overpower_clear` / `light_palace_clear` はlegacy sourceとして残すが、現行祭壇導線からは使用しない。
- アラン裏切り後は地下牢の生存者保護／宮殿解放を経て、雷の要塞への急報に接続する。
...
### 2026-08-20 光の宮殿回想 Phase 20 描画・操作整理

- 3F六芒星の9分割Phaser描画で、crop後のscaleをcrop矩形基準へ修正。X17/Y16・9タイル・床より上／壁人物より下の仕様を維持する。
- ジャスパー／ヴェルド登場時の大型深淵渦を、MAP床に下半分が潜らないカットシーン前景深度へ変更。
- ヴェルド初戦後のクロードはX18/Y24まで右へ走ってから北上する。
- ストーリーHEALの汎用回復ログと、階段へ乗っただけで出る汎用ロック／存在ログを既定で抑止。
