...
const MenuStatus = {
    ...
    render: () => {
        ...
        // モンスター図鑑の計算。イベント専用variantは登録数・分母の双方から除外する。
        const bestiaryMonsters = (typeof DB !== 'undefined' && Array.isArray(DB.MONSTERS))
            ? DB.MONSTERS.filter(monster => monster?.bestiaryExcluded !== true)
            : [];
        const bestiaryIds = new Set(bestiaryMonsters.map(monster => Number(monster.id)));
        const bookCount = App.data.book
            ? (App.data.book.monsters || []).filter(id => bestiaryIds.has(Number(id))).length
            : 0;
        const totalMonsters = bestiaryMonsters.length;
