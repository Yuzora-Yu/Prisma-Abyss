...
for (const key of ['2-2', '2-3', '2-4', '2-5', '2-6', '2-7', '4-1', '4-2', '4-3', '4-4', '4-5']) {
...
const sophiaPostClearState = waterCitySophia.states?.find(state => state.stateId === 'water_city_post_clear');
...
assert(sophiaQuestState?.when?.requiredFlag === 'legacySophiaAlanQuestEnabled',
    'Legacy Sophia/Alan joint recruitment state must stay disabled after their story routes split.');
assert(waterCityAlan?.states?.[0]?.when?.requiredFlag === 'legacySophiaAlanQuestEnabled',
    'Legacy Water City Alan recruitment actor must stay disabled after Alan becomes a story recruit.');
...
assert(sophiaPostClearState?.when?.requiredFlag === 'waterCityCleared' && sophiaPostClearState?.when?.missingFlag === 'waterCityPostClearTalked' &&
    Number(sophiaPostClearState?.placement?.x) === 20 && Number(sophiaPostClearState?.placement?.y) === 13,
    'Sophia needs a post-clear breathing-space state at the Water City plaza.');
assert(visibleWaterActors({ waterCityIntroCleared: true, waterCityCleared: true }).some(entry => entry.actorId === 'sophia_water_city' && entry.x === 20 && entry.y === 13),
    'Water City plaza must show Sophia once after the city is liberated.');
assert(!visibleWaterActors({ waterCityIntroCleared: true, waterCityCleared: true, waterCityPostClearTalked: true }).some(entry => entry.actorId === 'sophia_water_city' && entry.x === 20 && entry.y === 13),
    'Post-clear Sophia plaza state must disappear after the breathing-space conversation.');
assert(!visibleWaterActors({ waterCityIntroCleared: true, waterCityCleared: true, thunderFortCleared: true, waterCityPostClearTalked: true, rexnoteRouteKnown: true })
    .some(entry => entry.actorId === 'sophia_alan_seabed_depths_2'),
    'Legacy Alan quest actor must not reappear after Thunder Fort clear.');
