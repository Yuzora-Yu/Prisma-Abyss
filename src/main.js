...
const App = {
    ...
    restoreSceneContextSnapshot: (snapshot) => {
        ...
        ['flags','worldState','storyCharacters','storyRewards','quests'].forEach(key => {
    ...
    normalizeWorldStateConditions: (value) => {
        ...
        if (typeof value === 'object') {
            ...
            return Object.entries(value).map(([key, expected]) => ({ key, value: expected, op: '===' }));
        }
        return [];
    },
...
スカイプリズムを1個消費します。`, () => {
    ...
    addStoryAlly: (charId, options = {}) => {
        ...
        const initialLevel = Math.max(1, Math.min(100, Number.isFinite(requestedInitialLevel) ? requestedInitialLevel : 1));
