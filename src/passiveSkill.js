...
PassiveSkill.TRAIT_BOOK_TRAIT_IDS = Object.freeze(
    Object.keys(PassiveSkill.MASTER).map(Number).filter(id => Number.isInteger(id) && !PassiveSkill.MASTER[id]?.bossOnly).sort((a, b) => a - b)
);
...
PassiveSkill.getEquipmentTraitCandidateIds = function(equipment = null) {
    ...
    return [...new Set(ids.map(Number))].filter(id => PassiveSkill.MASTER[id] && !PassiveSkill.MASTER[id].bossOnly && !forbidden.has(id));
};

PassiveSkill.getRandomTraitId = function(category = null) {
    let ids = Object.keys(PassiveSkill.MASTER).map(Number).filter(id => !PassiveSkill.MASTER[id]?.bossOnly);
    if (category) {
