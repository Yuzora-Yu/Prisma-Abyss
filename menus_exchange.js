...
const MenuExchange = {
    ...
    render: () => {
        ...
        const seenTutorials = window.TutorialModal && typeof window.TutorialModal.getSeenTutorials === 'function'
            ? window.TutorialModal.getSeenTutorials()
            : [];
        if (MenuExchange.activeTab === 'tutorials' && !seenTutorials.length) MenuExchange.activeTab = 'news';
        const isNews = MenuExchange.activeTab === 'news';
        const tutorialTab = seenTutorials.length ? `
                <button
    ...
    renderTutorialTab: () => {
        ...
        return `
            ...
            <div id="tutorial-list">
                ...
                `).join('') : '<div style="padding:20px; color:#777; font-size:12px; text-align:center;">まだ確認できるチュートリアルはありません。</div>'}
            </div>
        `;
    },
